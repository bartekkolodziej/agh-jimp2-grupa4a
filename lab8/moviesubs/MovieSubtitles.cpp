...
namespace moviesubs {
    ...
    void SubRipSubtitles::ConvertTimeToCorrectFormat(int delay, int time_seconds, int time_miliseconds,
                                                     std::stringstream *out) {
        *out << ':';
        if (time_miliseconds + delay < 1000) {
            if (time_seconds < 10) {
                if (time_miliseconds > 100)
                    *out << '0' + std::to_string(time_seconds) + ',' + std::to_string(time_miliseconds + delay);
                if (time_miliseconds < 100)
                    *out << '0' + std::to_string(time_seconds) + ",0" + std::to_string(time_miliseconds + delay);
                if (time_miliseconds < 10)
                    *out << '0' + std::to_string(time_seconds) + ",00" + std::to_string(time_miliseconds + delay);
            }
            if (time_seconds > 10) {
                if (time_miliseconds > 100)
                    *out << std::to_string(time_seconds) + ',' + std::to_string(time_miliseconds + delay);
                if (time_miliseconds < 100)
            ...
            if (time_seconds < 10) {
                ...
                if (time_miliseconds < 100)
                    *out << '0' + std::to_string(time_seconds) + ",0" + std::to_string(time_miliseconds);
                if (time_miliseconds < 10)
                    *out << '0' + std::to_string(time_seconds) + ",00" + std::to_string(time_miliseconds);
            }
            if (time_seconds > 10) {
                if (time_miliseconds > 100)
                    *out << std::to_string(time_seconds) + ',' + std::to_string(time_miliseconds);
                if (time_miliseconds < 100)
    ...
    int SubRipSubtitles::ConvertTimeToMiliseconds(std::string time) {
        ...
        time_miliseconds =
                atoi(hours.c_str()) * 3600000 + atoi(minutes.c_str()) * 60000 + atoi(seconds.c_str()) * 1000 +
                atoi(miliseconds.c_str());
    ...
    void SubRipSubtitles::IfEndEarlierThanStart(std::stringstream *in) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (text[i] == '-' && text[i + 1] == '-' && text[i + 2] == '>') {
                ...
                if (ConvertTimeToMiliseconds(time_start) > ConvertTimeToMiliseconds(time_end))
