        ...
        for(int col=0;col<=new_matrix.cols;col++){
            if(isAxised[col]== false){
                if(abs(new_matrix.matrix[row][col])>max){
                    max=fabs(new_matrix.matrix[row][col]);
                    int p=row;
                    int q=col;
                }
            }
...
void Matrix::print(){
    for(int row=0;row <= this->rows;row++){
        for(int col=0;col<=this->cols;col++){
            std::string number=std::string(this->matrix[row][col]);
            std::string spaces="";
            if(number.length()<8)for(int i=0;i<7-number.length();i++)spaces+=" ";
